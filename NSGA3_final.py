...
def nsga3(env,n_pop, n_vars,parent_ratio, crossover_rate, muation_rate, lamda_miu_ratio, generations):
    ...
    def init_individual():
        ind = toolbox.individual()  # 初始化权重
        setattr(ind, 'sigma', toolbox.init_sigma())  # 添加独立的变异率属性
        return ind
    ...
    toolbox.register("mate", tools.cxBlend, alpha= parent_ratio)  # 混合交叉操作中父代基因的混合程度
    toolbox.register("mutate", self_adaptive_mutation, low=-1.0, up=1.0)  # 自适应变异
...
def test(enemy_number, dir_name, algorithm, run):
    ...
    experiment_name = (f'solution_tuning/{enemy_number}/{algorithm}/{dir_name}/run{run}')
...
def main():
    ...
    for i in range(len(combinations)):
        ...
        if args.mode in ['test', 'full']:
            with open('Task2_score_final_nsga.txt', 'a') as file:
                for run in range(n_runs):  # num of runs
                    for algorithm in algorithms:
                        ...
                        for i in range(5):  # 5 repeats for each independent run
                             ...
                             print(sub_dir, run+1, "player_hp:", p_hp)
                             print(sub_dir, run+1, "enemy_hp:", e_hp)
                             count = sum(1 for p, e in zip(p_hp, e_hp) if p > e)
                             print( sub_dir, run+1, "win_hp:", count)
                             print(sub_dir, run+1, "sum_player_hp:", round(sum(p_hp), 2))
                             print(sub_dir, run+1, "total_gain_hp:", round((sum(p_hp) - sum(e_hp)), 2))
